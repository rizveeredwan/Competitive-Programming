...
struct Dfs{
    ...
    int cost = -1;
    int explored = 0;
    void algorithm(Graph g, int start, int goal, int max_bound){
        this->init(g);
        int cost;
        int i=0;
        for(i=0; i<=max_bound; i++){
            this->init(g);
            cost = this->recursion_dfs(g,start, goal, 0, i);
            if(this->goal_reached == true) {
                break;
            }
        }
        this->print(cost, start, goal,i);
    }
    ...
    void print(int cost, int start, int goal, int depth){
        cout<<"Iterative depth "<<depth<<endl;
    ...
    int recursion_dfs(Graph g, int u, int goal, int curr_level, int max_level){
        ...
        for(int i=0; i<g.edges[u].size(); i++){
            if(this->color[g.edges[u][i]] == 0) {
                ...
                local_cost = g.weight[u][i] + this->recursion_dfs(g,g.edges[u][i], goal, curr_level+1, max_level);
